...
local function spawnViewmodel(knife)
    print("[BloxStrike] spawnViewmodel iniciou")

    if spawned or not scriptRunning then
        print("[BloxStrike] bloqueado:", spawned, scriptRunning)
        return
    end

    local myModel = isAlive()
    if not myModel then
        print("[BloxStrike] jogador não está vivo")
        return
    end

    print("[BloxStrike] jogador está vivo")
    local myModel = isAlive()
    if not myModel the
    spawned = true
